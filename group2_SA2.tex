...
\begin{frame}{Objectives}
		...
		\large
				To obtain an assumption-robust
			...
			\begin{block}{Specific Objectives}
				%\small
				\begin{enumerate}
					\item To estimate survival empirically (Kaplan-Meier)
					\item To Characterise cumulative hazard (Nelson-Aalen)
					\item To test group differences with \alert{multiple
						weighted tests} (Log-Rank, Breslow,
					Tarone-Ware, Fleming-Harrington)
					\item To model covariate effects via Cox PH
					\item To diagnose, refine, and \alert{extend} the
					Cox model (log-log plots, time interactions)
					\item To synthesise all findings
				\end{enumerate}
			\end{block}
